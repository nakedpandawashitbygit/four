    ...
    xhr.onload = function() {
        if (xhr.status === 200) {
            ...
            try {
                ...
                if (response.status === 'error') {
                    showErrorModal(response.message); // Show error modal in case of error
                } else if (response.status === 'success') {
                    // Handle successful link creation
                    console.log('Short URL Created:', response.short_url);
                    // Optionally show a success message or update the UI with the new short URL
                    // Example: Add the new short URL to the list of created URLs dynamically
                    const newLinkHtml = `<p>New Short URL: <a href="/${response.short_url}" target="_blank">${response.short_url}</a></p>`;
                    document.querySelector('.links-cards').insertAdjacentHTML('afterbegin', newLinkHtml);
                    // Optionally clear the form
                    document.querySelector('form').reset();
                }
            } catch (e) {
                // Handle any parsing errors
                showErrorModal(`Unexpected response format. Error: ${e.message}, Raw response: ${xhr.responseText}`);
            }
        } else {
            alert('An error occurred while submitting the form.');
        }
