...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
	...
	$new_expiration_date = !empty($_POST['new_expiration_date']) ? filter_var($_POST['new_expiration_date'], FILTER_SANITIZE_STRING) : null;
    $new_password = !empty($_POST['new_password']) ? password_hash($_POST['new_password'], PASSWORD_BCRYPT) : null;


    // Log the incoming data
    logMessage("Attempting to edit short URL with ID: $id. New values - Long URL: $new_long_url, Short URL: $new_short_url, Title: $new_title, Expiration: $new_expiration_date", 'INFO');
    ...
    if ($count > 0) {
        ...
        $stmt->bind_param("sssssi", $new_long_url, $new_short_url, $new_title, $new_expiration_date, $new_password, $id);
        
        // Log the SQL query execution
        logMessage("Executing SQL query: UPDATE four SET long_url = $new_long_url, short_url = $new_short_url, title = $new_title, expiration_date = $new_expiration_date WHERE id = $id", 'INFO');
...
?>
    ...
    <body>
		...
		<div class="d-flex" style="align-items: stretch; width: 100%; overflow: hidden;">
			...
			<nav id="sidebar">
				...
				<ul class="list-unstyled components">
					...
					<li>
						<a href="logout.php"><img src="/img/buttons/logout.svg"><span>Logout</span></a>
					</li>
					<li>
		...
		<div class="d-flex" id="dasboard-content" style="flex: 1; overflow: hidden; margin-left:250px">
        
            <!-- New short url builder form -->
            <div class="builder-form" style="flex: 0 0 40%; padding: 20px; box-sizing: border-box; overflow-y: auto;">
                <div class="container">
                    <div class="col-md-12">
                                    ...
                                    <form method="POST" action="/dashboard">
										...
										<div id="passwordField" class="mb-3" style="display:none;">
											...
											<div class="input-group">
												<input type="password" class="form-control" id="link_password" name="link_password" placeholder="Your Password">
												<button class="btn btn-outline-secondary" type="button" id="togglePassword" onclick="toggleLinkPasswordVisibility()">
													👁️
												</button>
											</div>
										</div>
                                            ...
                                            <div class="card-content">
												...
												*/
												...
												<p class="card-text card-short-url d-flex align-items-center">
													...
													<!-- Cancel button -->
													<button type="button" id="card-button-cancel-<?php echo $row['id']; ?>" class="btn btn-outline-secondary btn-sm ms-2" onclick="cancelEditAll(<?php echo $row['id']; ?>)" style="display: none;">❌</button>
													<!-- Delete button -->
												...
												*/
												...
												<input type="password" name="new_password" class="password-input" id="password-input-<?php echo $row['id']; ?>" placeholder="Enter new password" style="display: none;">
												...
												*/
												...
												<?php
												/*
												<div class="password-input-group input-group mb-3" id="password-field-group-<?php echo $row['id']; ?>" style="display: none;">
													<!-- Input field for editing the password -->
													...
													<!-- Eye icon button to toggle password visibility -->
													<button type="button" class="btn btn-outline-secondary" id="toggle-password-visibility-<?php echo $row['id']; ?>" onclick="togglePasswordVisibility(<?php echo $row['id']; ?>)">👁️</button>
												</div>

												<!-- Icons for password actions -->
												<div id="password-icons-<?php echo $row['id']; ?>" style="display: none !important;" class="">
													<!-- Save button -->
													<button type="button" id="password-button-save-<?php echo $row['id']; ?>" class="btn btn-outline-primary btn-sm" onclick="savePassword(<?php echo $row['id']; ?>)">💾</button>
													<!-- Cancel button -->
													<button type="button" id="password-button-cancel-<?php echo $row['id']; ?>" class="btn btn-outline-secondary btn-sm" onclick="cancelPassword(<?php echo $row['id']; ?>)">❌</button>
												...
												*/
												...
												<div class="align-items-center mb-2" id="password-field-group-<?php echo $row['id']; ?>" style="display: none !important;"> <!-- d-flex class is controlled by Javascript -->
													...
													<div id="password-icons-<?php echo $row['id']; ?>" style="display: none !important;">
														<!-- Save button -->
														<button type="button" id="password-button-save-<?php echo $row['id']; ?>" class="btn btn-outline-primary btn-sm me-1" onclick="savePassword(<?php echo $row['id']; ?>)">💾</button>
														<!-- Cancel button -->
														<button type="button" id="password-button-cancel-<?php echo $row['id']; ?>" class="btn btn-outline-secondary btn-sm me-1" onclick="cancelPassword(<?php echo $row['id']; ?>)">❌</button>
														<!-- Delete button -->
														<button type="button" id="password-button-delete-<?php echo $row['id']; ?>" class="btn btn-outline-danger btn-sm" onclick="deletePassword(<?php echo $row['id']; ?>)">🗑️</button>
													</div>
												</div>
