...
// Check if a short URL exists for a different record
...
// Handle AJAX request to check uniqueness
...
/*if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['short_url'])) {
    ...
    header('Content-Type: application/json'); // Make sure to return a JSON response
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['edit_id'])) {
    header('Content-Type: application/json');  // Указываем, что будет возвращён JSON
