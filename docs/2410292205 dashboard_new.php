...
/*if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['edit_id'])) {
    ...
    if (empty($short_url) || empty($long_url)) {
        $error_message = 'Short URL and long URL cannot be empty.';
    }
    ...
    if ($stmt->execute()) {
		...
		echo json_encode(['status' => 'success', 'id' => $id, 'short_url' => $short_url]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['edit_id'])) {
    ...
    if (!$validated_url) {
        ...
        // Normalize both the input URL and the site URL for comparison
        //$normalized_long_url = normalizeUrl($long_url);
        //$normalized_site_url = normalizeUrl($site_url);
