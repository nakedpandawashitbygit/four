...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    ...
    if ($count > 0) {
        ...
        if ($stmt->execute()) {
            ...
            QRcode::png('https://lnk.monster/'.$new_short_url, $path);
            header('Location: dashboard.php');
            exit;
        }
        $stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['edit_id'])) {
    ...
    if ($stmt->execute()) {
        $id = $stmt->insert_id;
        require_once 'phpqrcode/qrlib.php';
        QRcode::png("https://lnk.monster/r.php?id=$id", "qrcodes/$id.png");
    } else {
...
?>
    ...
    <head>
        ...
        .card {
          position: relative;
          padding-top: 1px; /* Добавляем отступ сверху, чтобы кнопки не перекрывали контент */
        }
        ...
        .card-buttons button,
        .card-buttons a {
          flex-shrink: 0; /* Предотвращаем изменение размеров кнопок */
        }
        
        .card-buttons button,
        .card-buttons a {
          display: inline-block;
          white-space: nowrap; /* Отключаем перенос текста внутри кнопок */
        }
        </style>
        ...
        <nav class="navbar navbar-expand-lg navbar-custom">
            ...
            <img src="user_icon.png" alt="User Icon" class="img-fluid" style="width: 100%; height: 100%;">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end text-small" aria-labelledby="dropdownUser1">
                        ...
                        <li><a class="dropdown-item" href="logs/app_log.txt">Logs</a></li> <!-- Видно только админам -->
                        <?php endif; ?>
                    </ul>
                </div>
            </div>
            ...
            <div class="row">
              ...
              <input type="text" name="new_title" id="title-input-<?php echo $row['id']; ?>" value="<?php echo $row['title']; ?>" style="display: none;">
                                    ...
                                    <p class="card-text">
                                        ...
                                        <span id="short-url-text-<?php echo $row['id']; ?>"><a href="https://lnk.monster/<?php echo $row['short_url']; ?>" target="_blank"><?php echo $row['short_url']; ?></a></span>
                                    ...
                                    <!-- Переходы -->
                                    <p class="card-text"><strong>Переходов по короткой ссылке:</strong>
                                        <?php echo $row['short_count']; ?>
                                    </p>
                                    <p class="card-text"><strong>Переходов по QR-коду:</strong>
