...
// Function to validate the long URL
function validateLongUrl($url) {
    ...
    // Check if it's a valid URL using filter and domain structure
    if (!filter_var($url, FILTER_VALIDATE_URL) || !preg_match('/^https?:\/\/([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,6}(\/[a-zA-Z0-9-._~:\/?#[\]@!$&\'()*+,;%=]*)?$/', $url)) {
        return false; // Not valid
    }

    return $url; // Return the valid URL with scheme
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['edit_id'])) {
	...
	$long_url = trim($_POST['long_url']);  // Remove any extra spaces
    $validated_url = validateLongUrl($long_url);

    /*if (!$validated_url) {
        $error_message = 'Error: The long URL is not valid.';
    }

    $long_url = $validated_url; // Use the validated long URL

    // Prevent creating short URLs for links on our own domain
    if (strpos($long_url, $site_url) === 0) {
        $error_message = 'Error: Cannot create a short link for a link on your own domain.';
    }*/
	
	if (!$validated_url) {
        $error_message = 'The long URL is not valid.';
    } else {
        $long_url = $validated_url; // Use the validated long URL

        // Prevent creating short URLs for links on our own domain
        if (strpos($long_url, $site_url) === 0) {
            $error_message = 'Cannot create a short link for a link on your own domain.';
        }
    }

    // Short URL validation (at least 4 characters, alphanumeric)
    $short_url = filter_var(shortenUrl(), FILTER_SANITIZE_STRING);
    if (strlen($short_url) < 4 || !preg_match('/^[a-zA-Z0-9\-]+$/', $short_url)) {
        $error_message = 'Short URL must be at least 4 characters long and contain only letters, numbers, and hyphens.';
    }
    ...
    // If there's an error, return the error in JSON format for AJAX
    if (!empty($error_message)) {
                        ...
                        <div class="card mb-3">
                            <div class="card-body">
                                <div class="mb-4">
                                    ...
                                    <form id="create-link-form" method="POST" action="/dashboard">
                                        
                                        <div class="input-group mb-3">
                                            <input type="url" class="form-control" id="long_url" name="long_url" placeholder="Enter your link" required>
