...
if (!$conn->query("DELETE FROM four WHERE expiration_date IS NOT NULL AND expiration_date <= NOW()")) {
...
if (isset($_GET['delete'])) {
    ...
    // Prepared statement for deletion by id
    $stmt = $conn->prepare("DELETE FROM four WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();

    // Handling file deletion errors
    if (!unlink("qrcodes/$id.png")) {
        error_log("Failed to delete QR code image for ID: $id");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    ...
    // Ensure that the new short URL is unique
    ...
    if ($count > 0) {
        ...
        // Updating the record with new URL details
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['edit_id'])) {
    ...
    if (!empty($utm_params)) {
        // Check if '?' exists in the URL, and add '?' or '&'
        $long_url .= (strpos($long_url, '?') === false ? '?' : '&') . implode('&', $utm_params);
    }

    // Capture other form fields
    $short_url = shortenUrl(); // Ensure this function works
    $title = $_POST['title'] ?? 'Untitled'; // Ensure title is captured
    $comment = $_POST['comment'] ?? null;
    $expiration_date = $_POST['expiration_date'] ?? null;
    $link_password = $_POST['link_password'] ?? null;

    // Debug UTM parameters and title to verify correctness
    var_dump($utm_params); // Check if UTM params are built correctly
    var_dump($title); // Check if the title is captured correctly
...
// Securely fetching user-specific URLs
$user_id = $_SESSION['user_id'];
$stmt = $conn->prepare("SELECT * FROM four WHERE user_id = ? ORDER BY created_at DESC");
$stmt->bind_param("i", $user_id);
$stmt->execute();
$result = $stmt->get_result();  // Getting result set for the prepared statement
$stmt->close();
?>
    ...
    <head>
        ...
        <style>
            .navbar-custom {
              background-color: #0c6efc; /* Blue color for navbar */
            }
            .user-icon {
              width: 40px;
              height: 40px;
            ...
            .navbar-custom .container {
              max-width: 960px; /* Width of navbar */
            }
        </style>
    </head>
    ...
    <body>
        ...
        <div class="container mt-5">
            ...
            <div class="creator">
                <!-- Form for creating new short URL -->
                <div class="mb-4">
                    <form method="POST" action="dashboard_new.php" class="d-flex">
                        <input type="text" class="form-control me-2" id="long_url" name="long_url" placeholder="Enter URL" required>
                ...
                <div class="mb-3 d-flex align-items-center">
                    ...
                    <div class="form-check form-switch me-3">
                        ...
                        <label class="form-check-label" for="toggleExpiration">Expiration Date</label>
                    </div>
                    <div class="form-check form-switch me-3">
                    ...
                    <div class="form-check form-switch">
                        ...
                        <label class="form-check-label" for="toggleComment">Comment</label>
                    </div>
                </div>
                ...
                <div id="passwordField" class="mb-3" style="display:none;">
                    ...
                    <input type="password" class="form-control" id="link_password" name="link_password" placeholder="Enter password">
                </div>
                            ...
                            <div class="card mb-3">
                                <div class="card-body">
                                    <form method="POST" action="dashboard_new.php" id="edit-form-<?php echo $row['id']; ?>">
                                        ...
                                        <!-- Display QR code -->
                                        <p class="card-text">
                                            <img src="qrcodes/<?php echo $row['id']; ?>.png" alt="QR Code" class="img-fluid" style="width: 100px; height: 100px;">
                                        </p>
                                        ...
                                        <p class="card-text">
                                            ...
                                            <span id="password-text-<?php echo $row['id']; ?>"><?php echo $row['password'] ? '*****' : 'None'; ?></span>
                                            <input type="password" name="new_password" id="password-input-<?php echo $row['id']; ?>" value="<?php echo $row['password']; ?>" style="display: none;">
                                        </p>

                                        <!-- Link clicks -->
                                        <p class="card-text"><strong>Short URL Clicks:</strong>
                                            <?php echo $row['short_count']; ?>
                                        </p>
                                        <p class="card-text"><strong>QR Code Clicks:</strong>
        ...
        <script>
            function toggleEdit(id) {
              ...
              // Toggle visibility of fields for editing
            ...
            function cancelEdit(id) {
              toggleEdit(id); // Hide edit fields
            }
        </script>
