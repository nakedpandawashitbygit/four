...
ini_set('session.gc_maxlifetime', 8640000);  // 24 часа х10
session_set_cookie_params(8640000); // срок действия cookie сессии
...
// one function to rule them all (i mean both)
function validateAndNormalizeLongUrl($url) {
    ...
    // Validate the domain (exclude invalid domains)
    $host = parse_url($url, PHP_URL_HOST); // Extract the host
    if (!preg_match('/^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$/', $host)) {
        logMessage("Invalid domain in URL: $url", 'ERROR'); // Log the invalid domain
        return false; // Invalid domain
    }

    // Normalize: remove scheme, www., slashes, and convert to lowercase
    $normalized_url = preg_replace('/^https?:\/\//', '', $url); // Remove http:// or https://
    $normalized_url = preg_replace('/^www\./', '', $normalized_url); // Remove www.
    $normalized_url = strtolower($normalized_url); // Convert to lowercase
    $normalized_url = rtrim($normalized_url, '/'); // Remove trailing slash

    return $normalized_url; // Return the normalized URL
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
	...
	$validated_url = validateAndNormalizeLongUrl($new_long_url);
	if (!$validated_url) {
...
// Function to normalize the URL without the scheme
function normalizeUrlWithoutScheme($url) {
    // Remove 'http://', 'https://' or 'www.' from the URL
    $url = preg_replace('/^https?:\/\//i', '', $url); // Remove http:// or https:// (case-insensitive)
    $url = preg_replace('/^www\./i', '', $url);       // Remove 'www.' if present (case-insensitive)

    // Parse the URL to handle query strings and paths
    $parsed_url = parse_url($url);

    // Build the normalized URL
    $normalized_url = isset($parsed_url['host']) ? strtolower($parsed_url['host']) : ''; // Hostname in lowercase
    $normalized_url .= isset($parsed_url['path']) ? rtrim($parsed_url['path'], '/') : ''; // Path without trailing slash
    $normalized_url .= isset($parsed_url['query']) ? '?' . $parsed_url['query'] : '';    // Append query string if exists
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['edit_id'])) {
	...
	$validated_url = validateAndNormalizeLongUrl($long_url);
    ...
    if (!$validated_url) {
		...
		// $normalized_long_url = validateAndNormalizeLongUrl($long_url);
		$validated_site_url = validateAndNormalizeLongUrl($site_url);
        ...
        if (strpos($validated_url, $validated_site_url) === 0) {
		...
		<div class="d-flex" style="align-items: stretch; width: 100%; overflow: hidden;">
			...
			<nav id="sidebar">
				...
				<ul class="list-unstyled components">
					...
					<li>
						<a href="lk.php"><img src="/img/buttons/news.svg"><span>Profile</span></a>
					</li>
